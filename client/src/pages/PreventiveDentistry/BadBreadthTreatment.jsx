...
const BadBreadthTreatment = () => {
  return (
    <>
      <Banner data={banner_data.bad_breath_treatment} />
      {para_data.bad_breath_treatment.map((item) => (
        <Para key={item.para_title} data={item} />
      ))}
      {faq.bad_breath_treatment ? (
        <>
          <chakra.h2
            maxW={{
              base: "xl",
          ...
          {faq.bad_breath_treatment?.map((el, index) => (
...
export default BadBreadthTreatment;
