...
const JawPain = () => {
  return (
    <>
      <Banner data={banner_data.jaw_pain_treatment} />
      {para_data.jaw_pain_treatment.map((item) => (
        <Para key={item.para_title} data={item} />
      ))}
      {faq.jaw_pain_treatment ? (
        <>
          <chakra.h2
            maxW={{
              base: "xl",
          ...
          {faq.jaw_pain_treatment?.map((el, index) => (
